function [ ii_data, ii_cfg ] = ii_selectfixationsbytrial_WMLoads( ii_data, ii_cfg, epoch_chan, within_epochs, sel_mode, buffer_window,calibrate_targs,fix_channels )
...
for tt = 1:length(tu)
    ...
    if strcmpi(sel_mode,'last') % used for drift correction, calibration
        ...
        if ~isempty(epoch_begin) % in case this calibrate epoch doesnt exist in this trial
