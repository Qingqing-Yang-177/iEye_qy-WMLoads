function [ii_sess] = ii_combineresp(ii_trial,resp_labels)
...
% 
% ===============================
% Revised for combining different resp_epoch data
% 
% revised the r_num into resp_num, so we could index diff resp_epoch
...
if nargin < 3 || isempty(resp_labels)
    resp_labels = 1:length(ii_trial);
else
    % check to be sure length(run_labels) matches length(ii_trial)
    if length(resp_labels)~=length(ii_trial)
...
resp_num = nan(size(ii_trial{1}.i_sacc,1)*length(ii_trial),1);
