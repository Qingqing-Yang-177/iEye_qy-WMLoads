function [ii_data,ii_cfg] = ii_definetrial_WMLoads(ii_data,ii_cfg, c1, v1, c2, v2)
...
if ismember(c1,fieldnames(ii_data)) && ismember(c2,fieldnames(ii_data))
    ...
    if swhere(1) ~= 1
        ...
        ewhere1 = find(diff(chan2 == v2(1))==-1);
        ewhere2 = find(diff(chan2 == v2(2))==-1);
    
        eend = max(ewhere1(end),ewhere2(end));
    
%         if eend ~= ewhere(end)
%             error('definetrial casue error, please check.');
%         end
    else
       ewheretemp = swhere -1;
       ewhere = ewheretemp(2:end);
