...
% =================
% Adapted for Multi-target all report, Separate feedback experiment
% settings.
...
ii_init; % use if ii_import_edf.m, remember to revise the path inside.
...
% applied)
if strcmp(ii_params.task,'targ2_control') % for exp that has diff end epoch values
    [ii_data,ii_cfg] = ii_definetrial_WMLoads(ii_data,ii_cfg,...
...
if ~ismember('calibration',skip_steps)
    ...
    % TarY1,...,TarY4
%     for cali_epoch = ii_params.calibrate_epoch(1):ii_params.calibrate_epoch(end)
%         cali_id=find(cali_epoch==ii_params.calibrate_epoch,1,'first');
    for cali_id = 1:length(ii_params.calibrate_epoch)
        cali_epoch = ii_params.calibrate_epoch(cali_id);
        
        [ii_data,ii_cfg] = ii_selectfixationsbytrial_WMLoads(ii_data,ii_cfg,ii_params.epoch_chan,...
            cali_epoch,ii_params.calibrate_select_mode,ii_params.calibrate_window,...
        ...
        if ismember(ii_params.calibrate_mode,{'scale','rotate'})
            ...
            % for debugging make ii_params.calibrate_limits bigger 
            % line 156: tu = tu(1:length(tu)-1); % when run is not finished
            % line 158 numtrials-1 % when run is not finished
            [ii_data,ii_cfg] = ii_calibratebytrial_WMLoads_SepFb(ii_data,ii_cfg,{'X','Y'},...
                ii_params.calibrate_target,ii_params.calibrate_mode, ii_params.calibrate_limits,cali_id,ii_params.calibrate_epoch);
