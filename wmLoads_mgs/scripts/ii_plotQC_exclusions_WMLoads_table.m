function [fh] = ii_plotQC_exclusions_WMLoads_table(ii_trial,which_excl,fig_visible)
...
% ======================
% Adapted for Multi-target all-report exp settings, after run data merged
% into table

% Qingqing Yang, 6/15/2022
...
for ee = 1:length(all_excl)
    ...
    trial_exist = zeros(length(ii_trial.excl_trial),1)==0;
    for tu = 1:length(ii_trial.excl_trial)
        trial_exist(tu)=isempty(find(10==ii_trial.excl_trial{tu}, 1));
    end
    this_excl= trial_exist&this_excl;
    pretrial_excl_rate=mean(this_excl)/mean(trial_exist);
    bar(ee,100*pretrial_excl_rate,'FaceColor',thiscolor);
    clear this_excl;
end
...
this_excl= trial_exist&this_excl;
all_excl_rate = mean(this_excl)/mean(trial_exist); 

bar(1,100*all_excl_rate,'EdgeColor',[0 0 0],'FaceColor',[0 0 0],'LineWidth',1.5);
clear this_excl;
...
this_excl= trial_exist&this_excl;
spec_excl_rate = mean(this_excl)/mean(trial_exist);

bar(2,100*spec_excl_rate,'EdgeColor',[0 0 0],'FaceColor',[1 1 1],'LineWidth',1.5);
...
if isfield(ii_trial,'r_num')
    ...
    for rr = 1:length(ru)
        ...
        ln=length({ii_trial.excl_trial{ii_trial.r_num==ru(rr)}});
        trial_exist = zeros(ln,1)==0;
        trial_exist=~cellfun(@any,cellfun(@(a) ismember(10,a),{ii_trial.excl_trial{ii_trial.r_num==ru(rr)}},'UniformOutput',false));
        
        this_excl_all = trial_exist&this_excl_all;
        this_excl_which = trial_exist&this_excl_which;
        
        excl_all_rate = mean(this_excl_all)/mean(trial_exist);
        excl_which_rate = mean(this_excl_which)/mean(trial_exist);
        
        excl_by_run(rr,1) = 100*excl_all_rate;
        excl_by_run(rr,2) = 100*excl_which_rate;
