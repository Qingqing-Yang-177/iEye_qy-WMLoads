function [ ii_data, ii_cfg ] = ii_calibratebytrial_WMLoads( ii_data, ii_cfg, chan_names, calib_targets, calib_mode, adj_limit,cali_id,calibrate_epoch )
...
% ===================
% Adapted for Multi-target all-report Separate feedback experiment settings
% Added ii_cfg.calibrate.allamt for 4 targets each trial, trial by 2*target
% Added ii_cfg.calibrate.allerr for 4 targets each trial, trail by target
% Added ii_cfg.calibrate.alladj for diff epoch of calibration
% ii_cfg.calibrate.amt takes the min-error target's X and Y amt each trail
% ii_cfg.calibrate.err takes the min error each trail within allerr

% Qingqing Yang, 6/7/2022
...
if ismember('calibrate',fieldnames(ii_cfg))
    warning('iEye:ii_calibratebytrial:alreadyCalibrated','CALIBRATE field already present in ii_cfg; will overwrite, except for calibrate.adj and calibrate.target.');
end


tu = unique(ii_cfg.trialvec(ii_cfg.trialvec~=0));
% tu = tu(1:length(tu)-1); % For debugging

% ii_cfg.numtrials = ii_cfg.numtrials-1; % For debugging
% get coords to calibrate to trials by 2 {X,Y}
calibrate_to = nan(ii_cfg.numtrials,4*length(chan_names));
...
if cali_id ==1 % only set it during the first epoch calibration
    ii_cfg.calibrate.target=calibrate_to;
end
ii_cfg.calibrate.fcn = 'ii_calibratebytrial_WMLoads.m';
...
ii_cfg.calibrate.allamt = nan(ii_cfg.numtrials,4*length(chan_names));
ii_cfg.calibrate.allerr = nan(ii_cfg.numtrials,4);
ii_cfg.calibrate.adj_limits = adj_limit;
ii_cfg.calibrate.alladj = zeros(ii_cfg.numtrials,length(calibrate_epoch))==1;
if cali_id == 1
    ii_cfg.calibrate.adj = zeros(ii_cfg.numtrials,1)==1; % logical, whether adjustment was performed on this trial
end
...
if strcmpi(calib_mode,'scale')
    ...
    for tt = 1:length(tu) % in each trial
        this_err = nan(length(chan_names),4);
        for pp = 1:4 % 4 for potential targets
            for cc = 1:length(chan_names)  % 2 for {X,Y}
                if cc == 1
                    qq = 2*pp-1;
                else
                    qq = 2*pp;
                ...
                if sum(ii_cfg.sel & tr_idx) > 0
                    if calibrate_to(tt,qq)==0
                        ...
                        adj_by = nanmean(ii_data.(sprintf('%s_fix',chan_names{cc}))(tr_idx & ii_cfg.sel) )/calibrate_to(tt,qq);

                        % difference fix - targ
%                         nanmean(ii_data.(sprintf('%s_fix',chan_names{cc}))(tr_idx & ii_cfg.sel) ) 
%                         calibrate_to(tt,qq)
                        this_err(cc,pp) = nanmean(ii_data.(sprintf('%s_fix',chan_names{cc}))(tr_idx & ii_cfg.sel) ) - calibrate_to(tt,qq); 
                    end
                else
                ...
                % for each trial, each chan, each target, get a adj_by (8)
                ii_cfg.calibrate.allamt(tt,qq) = adj_by;
                clear adj_by;
                
            end % end of {X,Y}
           
        end % end of target 
        % for each trial, each target, get an error (4)
        ii_cfg.calibrate.allerr(tt,1:4)=nan(1,4);
        ii_cfg.calibrate.allerr(tt,1:4) = sqrt(sum(this_err.^2)); 
        clear this_err; 
    end
    clear tt;clear cc;clear pp;
    
    % Now pick the first fixation is for which target
    for tt = 1:length(tu) % in each trial
        [min_err,min_errid]=min(ii_cfg.calibrate.allerr(tt,1:4));
        ii_cfg.calibrate.err(tt)=min_err;
        for cc = 1:length(chan_names)  % 2 for {X,Y}
            if cc ==1
                pp = min_errid*2-1;
            else
            ...
            ii_cfg.calibrate.amt(tt,cc)= ii_cfg.calibrate.allamt(tt,pp)
        end
    end
    clear tt; clear cc; clear pp;
        ...
        for tt = 1:length(tu)
            ...
            tr_idx = ii_cfg.trialvec==tu(tt);
                ...
                % within spec (ii_cfg.calibrate.err(tt) <= adj_limits)
                    ...
                    ii_cfg.calibrate.alladj(tt,cali_id)= 1==1;
